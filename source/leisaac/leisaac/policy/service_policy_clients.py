...
# --- {'left_wrist': (480, 640), 'right_wrist': (480, 640), 'top': (480, 640), 
# 'stereo_left': (720, 1280), 'stereo_right': (720, 1280)} 
# ['left_wrist', 'right_wrist', 'top', 'stereo_left', 'stereo_right']
    ...
    def _send_observation(self, observation_dict: dict):
        ...
        #         # try sensible fallbacks only if those keys exist in observation_dict
        #         if 'stereo_left' in key and 'left_wrist' in observation_dict and observation_dict['left_wrist'] is not None:
        #             raw = observation_dict['left_wrist'].cpu().numpy().astype(np.uint8)[0]
        #         if 'stereo_right' in key and 'right_wrist' in observation_dict and observation_dict['right_wrist'] is not None:
        #             raw = observation_dict['right_wrist'].cpu().numpy().astype(np.uint8)[0]

        #     # provide both plain camera key (e.g. 'left_wrist') and the
        #     # namespaced feature key 'observation.images.<camera_key>' so
        #     # servers that expect either form will find the image.
        #     raw_observation[f"{key}"] = raw
        #     raw_observation[f"observation.images.{key}"] = raw
