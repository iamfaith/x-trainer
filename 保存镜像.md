...
# 启动推理服务，监听 0.0.0.0
CMD ["/workspace/isaaclab/_isaac_sim/python.sh", "-m", "lerobot.async_inference.policy_server", "--host=0.0.0.0", "--port=5555", "--fps=30"]
...
如果有报错：修改x-trainer\source\leisaac\leisaac\policy\lerobot\__init__.py

helpers_path = "lerobot.scripts.server.helpers"
# helpers_path = "lerobot.async_inference.helpers"

5. 导出镜像：
docker save -o dobot_0001_submission_v1.tar dobot_0001_submission:v1
 上传dobot_0001_submission_v1.tar到网盘（最好附加一个演示视频）
